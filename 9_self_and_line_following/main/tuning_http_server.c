...
static esp_err_t tuning_pid_post_handler(httpd_req_t *req)
{
    ...
    if (!cJSON_HasObjectItem(root, "kp_pitch") || !cJSON_HasObjectItem(root, "ki_pitch") || !cJSON_HasObjectItem(root, "kd_pitch") || !cJSON_HasObjectItem(root, "kp_yaw") || !cJSON_HasObjectItem(root, "ki_yaw") || !cJSON_HasObjectItem(root, "kd_yaw") || !cJSON_HasObjectItem(root, "setpoint"))
    {
        ESP_LOGE(TAG, "invalid json response");
        return ESP_FAIL;
    }
    
    pid_constants.kp_pitch = (float)cJSON_GetObjectItem(root, "kp_pitch")->valuedouble;
    pid_constants.ki_pitch = (float)cJSON_GetObjectItem(root, "ki_pitch")->valuedouble;
    pid_constants.kd_pitch = (float)cJSON_GetObjectItem(root, "kd_pitch")->valuedouble;
    pid_constants.kp_yaw = (float)cJSON_GetObjectItem(root, "kp_yaw")->valuedouble;
    pid_constants.ki_yaw = (float)cJSON_GetObjectItem(root, "ki_yaw")->valuedouble;
    pid_constants.kd_yaw = (float)cJSON_GetObjectItem(root, "kd_yaw")->valuedouble;
